...
<body <?php body_class();?>>
	...
	<?php get_template_part(fire_department_get_file_slug('sidebar_outer.php')); ?>

	<?php
		$body_style  = fire_department_get_custom_option('body_style');
		$class = $style = '';
		if (fire_department_get_custom_option('bg_custom')=='yes' && ($body_style=='boxed' || fire_department_get_custom_option('bg_image_load')=='always')) {
			if (($img = fire_department_get_custom_option('bg_image_custom')) != '')
				$style = 'background: url('.esc_url($img).') ' . str_replace('_', ' ', fire_department_get_custom_option('bg_image_custom_position')) . ' no-repeat fixed;';
			else if (($img = fire_department_get_custom_option('bg_pattern_custom')) != '')
				$style = 'background: url('.esc_url($img).') 0 0 repeat fixed;';
			else if (($img = fire_department_get_custom_option('bg_image')) > 0)
				$class = 'bg_image_'.($img);
			else if (($img = fire_department_get_custom_option('bg_pattern')) > 0)
				$class = 'bg_pattern_'.($img);
			if (($img = fire_department_get_custom_option('bg_color')) != '')
	...
	<div class="body_wrap<?php echo !empty($class) ? ' '.esc_attr($class) : ''; ?>"<?php echo !empty($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
		...
		<?php
		$video_bg_show = fire_department_get_custom_option('show_video_bg')=='yes';
		$youtube = fire_department_get_custom_option('video_bg_youtube_code');
		$video   = fire_department_get_custom_option('video_bg_url');
		$overlay = fire_department_get_custom_option('video_bg_overlay')=='yes';
		if ($video_bg_show && (!empty($youtube) || !empty($video))) {
			if (!empty($youtube)) {
				...
				$ext = fire_department_get_file_ext($video);
		...
		<div class="page_wrap">
			...
			<?php
			$top_panel_style = fire_department_get_custom_option('top_panel_style');
			$top_panel_position = fire_department_get_custom_option('top_panel_position');
			$top_panel_scheme = fire_department_get_custom_option('top_panel_scheme');
			// Top panel 'Above' or 'Over'
			if (in_array($top_panel_position, array('above', 'over'))) {
				fire_department_show_post_layout(array(
					'layout' => $top_panel_style,
					'position' => $top_panel_position,
					'scheme' => $top_panel_scheme
					), false);
				// Mobile Menu
				get_template_part(fire_department_get_file_slug('templates/headers/_parts/header-mobile.php'));
			}

			// Slider
			get_template_part(fire_department_get_file_slug('templates/headers/_parts/slider.php'));
			...
			$show_title = fire_department_get_custom_option('show_page_title')=='yes';
			$show_navi = apply_filters('fire_department_filter_show_post_navi', false);
			$show_breadcrumbs = fire_department_get_custom_option('show_breadcrumbs')=='yes';
						...
						<div class="content_wrap">
							...
							if ($show_title) {
								if ($show_navi) {
									...
									?><h1 class="page_title"><?php echo strip_tags(fire_department_get_blog_title()); ?></h1><?php
								}
							}
							if ($show_breadcrumbs) {
								?><div class="breadcrumbs"><?php if (!is_404()) fire_department_show_breadcrumbs(); ?></div><?php
							}
							?>
						</div>
					</div>
			...
			<div class="page_content_wrap page_paddings_<?php echo esc_attr(fire_department_get_custom_option('body_paddings')); ?>">

				<?php
				// Content and sidebar wrapper
				if ($body_style!='fullscreen') fire_department_open_wrapper('<div class="content_wrap">');
				
				// Main content wrapper
				fire_department_open_wrapper('<div class="content">');
				?>