...
if ( !function_exists( 'fire_department_template_services_5_theme_setup' ) ) {
	add_action( 'fire_department_action_before_init_theme', 'fire_department_template_services_5_theme_setup', 1 );
	function fire_department_template_services_5_theme_setup() {
		fire_department_add_template(array(
			'layout' => 'services-5',
			'template' => 'services-5',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 5/', 'fire-department'),
...
// Template output
if ( !function_exists( 'fire_department_template_services_5_output' ) ) {
	function fire_department_template_services_5_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = fire_department_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
					if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
						?><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php fire_department_show_layout($html); ?></a><?php
					} else
						fire_department_show_layout($html);
				} else {
					?>
				...
				<div class="sc_services_item_content">
					...
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
							?><h4 class="sc_services_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php fire_department_show_layout($post_data['post_title']); ?></a></h4><?php
						} else {
							?><h4 class="sc_services_item_title"><?php fire_department_show_layout($post_data['post_title']); ?></h4><?php
						}
					}
					?>
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !fire_department_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore"><?php fire_department_show_layout($post_options['readmore']); ?><span class="icon-right"></span></a><?php
							}
						}
						?>
					</div>
