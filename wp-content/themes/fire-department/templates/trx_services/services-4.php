...
if ( !function_exists( 'fire_department_template_services_4_theme_setup' ) ) {
	add_action( 'fire_department_action_before_init_theme', 'fire_department_template_services_4_theme_setup', 1 );
	function fire_department_template_services_4_theme_setup() {
		fire_department_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'fire-department')
		));
	}
}

// Template output
if ( !function_exists( 'fire_department_template_services_4_output' ) ) {
	function fire_department_template_services_4_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				?><span class="sc_services_item_title"><?php fire_department_show_layout($post_data['post_title']); ?></span><?php
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?></a><?php
				}
				?>
