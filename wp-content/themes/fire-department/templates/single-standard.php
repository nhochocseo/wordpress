...
if ( !function_exists( 'fire_department_template_single_standard_theme_setup' ) ) {
    add_action( 'fire_department_action_before_init_theme', 'fire_department_template_single_standard_theme_setup', 1 );
    function fire_department_template_single_standard_theme_setup() {
        fire_department_add_template(array(
            'layout' => 'single-standard',
            'mode'   => 'single',
            'need_content' => true,
            'need_terms' => true,
            'title'  => esc_html__('Single standard', 'fire-department'),
...
// Template output
if ( !function_exists( 'fire_department_template_single_standard_output' ) ) {
    function fire_department_template_single_standard_output($post_options, $post_data) {
        ...
        if ($show_title && $post_options['location'] == 'center' && fire_department_get_custom_option('show_page_title')=='no') {
            ?>
            <<?php echo esc_html($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php fire_department_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
            <?php
        }
        ...
        if (!$sidebar_present) fire_department_close_wrapper();	// .post_item
