...
if ( !function_exists( 'fire_department_template_single_team_theme_setup' ) ) {
	add_action( 'fire_department_action_before_init_theme', 'fire_department_template_single_team_theme_setup', 1 );
	function fire_department_template_single_team_theme_setup() {
		fire_department_add_template(array(
			'layout' => 'single-team',
			'mode'   => 'single_team',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'fire-department'),
...
// Template output
if ( !function_exists( 'fire_department_template_single_team_output' ) ) {
	function fire_department_template_single_team_output($post_options, $post_data) {
		...
		if (!$post_data['post_protected'] && (
			...
			<section class="post_featured single_team_post_featured">
				...
				if (is_array($soc_list) && count($soc_list)>0) {
					...
					if (!empty($soc_str)) {
						?><div class="socials_single_team"><?php fire_department_show_layout(fire_department_sc_socials(array('size'=>"tiny", 'shape'=>'round', 'socials'=>$soc_str))); ?></div><?php
					}
				}
				?>
			</section>
			...
			<section class="single_team_post_description">
				<h2 class="team_title"><?php fire_department_show_layout($post_data['post_title']); ?></h2>
				<h6 class="team_position"><?php fire_department_show_layout($post_meta['team_member_position']); ?></h6>
				<div class="team_meta"><?php
					if (!empty($post_meta['team_member_email'])) {
						?><p><?php esc_attr_e('E-mail: ', 'fire-department'); ?><?php fire_department_show_layout($post_meta['team_member_email']); ?></p><?php
					}
					if (!empty($post_meta['team_member_bday'])) {
						?><p><?php esc_attr_e('Date Of Birth: ', 'fire-department'); ?><?php fire_department_show_layout($post_meta['team_member_bday']); ?></p><?php
					}
				?></div><?php
				...
				if (!empty($post_meta['team_member_brief_info'])) {
					...
					<div class="team_brief_info">
						<h5 class="team_brief_info_title"><?php esc_attr_e('Brief info', 'fire-department'); ?></h5>
		...
		if ($show_title && $post_options['location'] != 'center' && fire_department_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="name" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php fire_department_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
			<?php
		}
		...
		if ($post_data['post_protected']) { 
			...
			if ( fire_department_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'fire-department'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
