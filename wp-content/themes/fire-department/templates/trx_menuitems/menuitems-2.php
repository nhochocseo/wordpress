...
if ( !function_exists( 'fire_department_template_menuitems_2_theme_setup' ) ) {
	add_action( 'fire_department_action_before_init_theme', 'fire_department_template_menuitems_2_theme_setup', 1 );
	function fire_department_template_menuitems_2_theme_setup() {
		fire_department_add_template(array(
			'layout' => 'menuitems-2',
			'template' => 'menuitems-2',
			'mode'   => 'menuitems_single',
			'title'  => esc_html__('MenuItems /Style 2/', 'fire-department'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'fire-department'),
			'w'		 => 1170,
			'h'		 => 659
		));
	}
...
// Template output
if ( !function_exists( 'fire_department_template_menuitems_2_output' ) ) {
	function fire_department_template_menuitems_2_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_options['menuitem_image']) {
					...
					fire_department_show_layout($post_options['menuitem_image']);
					if ( $post_options['menuitem_spicylevel'] != 'inherit' && $post_options['menuitem_spicylevel'] != '0' && $post_options['menuitem_spicylevel'] != 'NaN' ) {
						?>
						<div class="sc_menuitem_spicy menuitem_spicylevel_<?php fire_department_show_layout($post_options['menuitem_spicylevel']); ?> ">
							<span class="icon-fire"></span> Spicy Level <?php fire_department_show_layout($post_options['menuitem_spicylevel']); ?>/5
						</div>
						<?php
					}
					?></div><?php
				...
				if ( $post_data['post_title'] || ( $post_options['menuitem_price'] != 'inherit' && strlen($post_options['menuitem_price']) != 0 ) ) {
					...
					<div class="sc_menuitem_box_title">
						...
						if ($post_data['post_title']) {
							?><div class="sc_menuitem_title"><strong><?php fire_department_show_layout($post_data['post_title']);?></strong></div><?php
						}
						if ( $post_options['menuitem_price'] != 'inherit' && strlen($post_options['menuitem_price']) != 0 ) {
							?><div class="sc_menuitem_price"><?php fire_department_show_layout($post_options['menuitem_price']); ?></div><?php
						}
						?>
						<div class="cL"></div>
					</div>
				...
				if ($post_data['post_content']) {
					...
					<div class="sc_menuitem_content">
						...
						<?php fire_department_show_layout($post_data['post_content']); ?>
					</div>
					<?php
				}
				...
				if (   ( $post_options['menuitem_calories'] != 'inherit'		&& strlen($post_options['menuitem_calories']) != 0 )
					...
					<div class="sc_menuitem_nutritions">
						...
						<ul class="sc_menuitem_nutritions_list">
							<?php
							if ( $post_options['menuitem_calories'] != 'inherit' && strlen($post_options['menuitem_calories']) != 0 )
								echo '<li>' . esc_html__('Calories: ', 'fire-department') . '<span>' . trim($post_options['menuitem_calories']) . esc_html__('Kcal', 'fire-department') . '</span></li>';
							if ( $post_options['menuitem_cholesterol'] != 'inherit' && strlen($post_options['menuitem_cholesterol']) != 0 ) {
								echo '<li>' . esc_html__('Cholesterol: ', 'fire-department') . '<span>' . trim($post_options['menuitem_cholesterol']) . esc_html__('mg', 'fire-department') . '</span></li>';
							}
							if ( $post_options['menuitem_fiber'] != 'inherit' && strlen($post_options['menuitem_fiber']) != 0 ) {
								echo '<li>' . esc_html__('Fiber: ', 'fire-department') . '<span>' . trim($post_options['menuitem_fiber']) . esc_html__('g', 'fire-department') . '</span></li>';
							}
							if ( $post_options['menuitem_sodium'] != 'inherit' && strlen($post_options['menuitem_sodium']) != 0 ) {
								echo '<li>' . esc_html__('Sodium: ', 'fire-department') . '<span>' . trim($post_options['menuitem_sodium']) . esc_html__('mg', 'fire-department') . '</span></li>';
							}
							if ( $post_options['menuitem_carbohydrates'] != 'inherit' && strlen($post_options['menuitem_carbohydrates']) != 0 ) {
								echo '<li>' . esc_html__('Carbohydrates: ', 'fire-department') . '<span>' . trim($post_options['menuitem_carbohydrates']) . esc_html__('g', 'fire-department') . '</span></li>';
							} 
							if ( $post_options['menuitem_fat'] != 'inherit' && strlen($post_options['menuitem_fat']) != 0 ) { 
								echo '<li>' . esc_html__('Fat: ', 'fire-department') . '<span>' . trim($post_options['menuitem_fat']) . esc_html__('g', 'fire-department') . '</span></li>';
							}
							if ( $post_options['menuitem_protein'] != 'inherit' && strlen($post_options['menuitem_protein']) != 0 ) {
								echo '<li>' . esc_html__('Protein: ', 'fire-department') . '<span>' . trim($post_options['menuitem_protein']) . esc_html__('g', 'fire-department') . '</span></li>';
							}
							?>
						</ul>
						<div class="cL"></div>
				...
				<div class="sc_menuitem_more">
					<?php if ( $post_options['menuitem_product'] != 'inherit' ) { ?>
						<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small margin_right_tiny" href="<?php echo get_permalink($post_options['menuitem_product']); ?>"><?php esc_html_e('ORDER', 'fire-department'); ?></a>
					<?php } ?>
					<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small" href="<?php echo esc_url($post_data['post_comments_link']); ?>"><?php esc_html_e('POST COMMENT', 'fire-department'); ?></a>
					<div class="cL"></div>
				</div>
