...
// Theme init
if (!function_exists('fire_department_widget_popular_posts_theme_setup')) {
	add_action( 'fire_department_action_before_init_theme', 'fire_department_widget_popular_posts_theme_setup', 1 );
	function fire_department_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('fire_department_exists_visual_composer') && fire_department_exists_visual_composer())
			add_action('fire_department_action_shortcodes_list_vc','fire_department_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('fire_department_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'fire_department_widget_popular_posts_load' );
	function fire_department_widget_popular_posts_load() {
		register_widget('fire_department_widget_popular_posts');
	}
}

// Widget Class
class fire_department_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'fire-department'));
		parent::__construct( 'fire_department_widget_popular_posts', esc_html__('Fire Department - Most Popular & Commented Posts', 'fire-department'), $widget_ops );

		// Add thumb sizes into list
		fire_department_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'fire-department') ) );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? fire_department_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = fire_department_storage_get('options_prefix').'_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = fire_department_storage_get('options_prefix').'_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'fire-department'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'fire-department'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'fire-department'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Number posts to show:', 'fire-department'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show post image:', 'fire-department'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show', 'fire-department'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_0"><?php esc_html_e('Hide', 'fire-department'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show post author:', 'fire-department'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show', 'fire-department'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_0"><?php esc_html_e('Hide', 'fire-department'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show post date:', 'fire-department'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show', 'fire-department'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_0"><?php esc_html_e('Hide', 'fire-department'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show post counters:', 'fire-department'); ?></label><br />
...
*/
if ( !function_exists( 'fire_department_sc_widget_popular_posts' ) ) {
	function fire_department_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'fire_department_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (fire_department_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, fire_department_prepare_widgets_args(fire_department_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('fire_department_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	fire_department_require_shortcode("trx_widget_popular_posts", "fire_department_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('fire_department_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('fire_department_action_shortcodes_list_vc','fire_department_widget_popular_posts_reg_shortcodes_vc');
	function fire_department_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", 'fire-department'),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", 'fire-department') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", 'fire-department'),
						"description" => wp_kses_data( __("Most popular tab title", 'fire-department') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", 'fire-department'),
						"description" => wp_kses_data( __("Most commented tab title", 'fire-department') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", 'fire-department'),
						"description" => wp_kses_data( __("Most liked tab title", 'fire-department') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "number",
						"heading" => esc_html__("Number posts to show", 'fire-department'),
						"description" => wp_kses_data( __("How many posts display in widget?", 'fire-department') ),
						"admin_label" => true,
						"class" => "",
						"value" => "4",
						"type" => "textfield"
					...
					array(
						...
						"description" => wp_kses_data( __("Select parent category. If empty - show posts from any category", 'fire-department') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show post's image", 'fire-department'),
						"description" => wp_kses_data( __("Do you want display post's featured image?", 'fire-department') ),
					...
					array(
						...
						"heading" => esc_html__("Show post's author", 'fire-department'),
						"description" => wp_kses_data( __("Do you want display post's author?", 'fire-department') ),
						"group" => esc_html__('Details', 'fire-department'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's date", 'fire-department'),
						"description" => wp_kses_data( __("Do you want display post's publish date?", 'fire-department') ),
						"group" => esc_html__('Details', 'fire-department'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's counters", 'fire-department'),
						"description" => wp_kses_data( __("Do you want display post's counters?", 'fire-department') ),
						"group" => esc_html__('Details', 'fire-department'),
						"class" => "",
						"std" => 1,
