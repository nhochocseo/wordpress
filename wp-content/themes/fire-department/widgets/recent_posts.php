...
// Theme init
if (!function_exists('fire_department_widget_recent_posts_theme_setup')) {
	add_action( 'fire_department_action_before_init_theme', 'fire_department_widget_recent_posts_theme_setup', 1 );
	function fire_department_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('fire_department_exists_visual_composer') && fire_department_exists_visual_composer())
			add_action('fire_department_action_shortcodes_list_vc','fire_department_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('fire_department_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'fire_department_widget_recent_posts_load' );
	function fire_department_widget_recent_posts_load() {
		register_widget('fire_department_widget_recent_posts');
	}
}

// Widget Class
class fire_department_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'fire-department'));
		parent::__construct( 'fire_department_widget_recent_posts', esc_html__('Fire Department - Recent Posts', 'fire-department'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'fire-department')), fire_department_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'fire_department_sc_widget_recent_posts' ) ) {
	function fire_department_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'fire_department_widget_recent_posts';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, fire_department_prepare_widgets_args(fire_department_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('fire_department_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	fire_department_require_shortcode("trx_widget_recent_posts", "fire_department_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('fire_department_widget_recent_posts_reg_shortcodes_vc')) {
	function fire_department_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'fire-department'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'fire-department') ),
