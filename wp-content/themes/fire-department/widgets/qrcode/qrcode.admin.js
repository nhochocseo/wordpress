...
jQuery(document).ready(function(){
	fire_department_qrcode_init();
	fire_department_color_picker();
});

function fire_department_qrcode_init() {
	jQuery('#widgets-right .widget_qrcode:not(.inited)').each(function() {
		...
		widget.find('input.iColorPicker:not(.colored)').each(function() {
			...
			if (obj.attr('id').indexOf('__i__') < 0) {
				...
				fire_department_set_color_picker(obj.attr('id'));
			}
		});
		widget.find('div.qrcode_tabs').tabs();
		widget.on('click', '.fld_button_draw', function() {
			fire_department_qrcode_update(widget);
		});
...
function fire_department_qrcode_update(widget) {
	fire_department_qrcode_show(widget, {
...
function fire_department_qrcode_show(widget, vc, opt) {
	if (opt.show_what==1) {
...
if (!window.fire_department_set_color_picker) {
	function fire_department_set_color_picker(id_picker) {
