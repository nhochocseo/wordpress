...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('fire_department_sc_tabs_theme_setup')) {
	add_action( 'fire_department_action_before_init_theme', 'fire_department_sc_tabs_theme_setup' );
	function fire_department_sc_tabs_theme_setup() {
		add_action('fire_department_action_shortcodes_list', 		'fire_department_sc_tabs_reg_shortcodes');
		if (function_exists('fire_department_exists_visual_composer') && fire_department_exists_visual_composer())
			add_action('fire_department_action_shortcodes_list_vc','fire_department_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('fire_department_sc_tabs')) {	
	function fire_department_sc_tabs($atts, $content = null) {
		...
		fire_department_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = fire_department_storage_get_array('sc_tab_data', 'titles');
		...
		fire_department_enqueue_script('jquery-effects-fade', false, array('jquery','jquery-effects-core'), null, true);
	
		$tabs_output .= $titles_output
			. '</ul>' 
			. ($content)
			.'</div>';
		return apply_filters('fire_department_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	fire_department_require_shortcode("trx_tabs", "fire_department_sc_tabs");
}


if (!function_exists('fire_department_sc_tab')) {	
	function fire_department_sc_tab($atts, $content = null) {
		if (fire_department_in_shortcode_blogger()) return '';
		extract(fire_department_html_decode(shortcode_atts(array(
			// Individual params
		...
		fire_department_storage_inc_array('sc_tab_data', 'counter');
		$counter = fire_department_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : fire_department_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = fire_department_storage_get_array('sc_tab_data', 'titles');
		...
		fire_department_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (fire_department_param_is_on(fire_department_storage_get_array('sc_tab_data', 'scroll'))
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(fire_department_storage_get_array('sc_tab_data', 'height') != '' ? fire_department_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">'
					: '')
				...
				. (fire_department_param_is_on(fire_department_storage_get_array('sc_tab_data', 'scroll'))
					...
					: '')
			. '</div>';
		return apply_filters('fire_department_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	fire_department_require_shortcode("trx_tab", "fire_department_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'fire_department_sc_tabs_reg_shortcodes' ) ) {
	//add_action('fire_department_action_shortcodes_list', 'fire_department_sc_tabs_reg_shortcodes');
	function fire_department_sc_tabs_reg_shortcodes() {
	
		fire_department_sc_map("trx_tabs", array(
			"title" => esc_html__("Tabs", 'fire-department'),
			"desc" => wp_kses_data( __("Insert tabs in your page (post)", 'fire-department') ),
			"decorate" => true,
			"container" => false,
			"params" => array(
				"style" => array(
					"title" => esc_html__("Tabs style", 'fire-department'),
					"desc" => wp_kses_data( __("Select style for tabs items", 'fire-department') ),
					"value" => 1,
					"options" => fire_department_get_list_styles(1, 2),
					"type" => "radio"
				),
				"initial" => array(
					"title" => esc_html__("Initially opened tab", 'fire-department'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'fire-department') ),
					"divider" => true,
					"value" => 1,
					"min" => 0,
					"type" => "spinner"
				...
				"scroll" => array(
					...
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'fire-department') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
			...
			"children" => array(
				"name" => "trx_tab",
				"title" => esc_html__("Tab", 'fire-department'),
				"desc" => wp_kses_data( __("Tab item", 'fire-department') ),
				"container" => true,
				"params" => array(
					"title" => array(
						"title" => esc_html__("Tab title", 'fire-department'),
						"desc" => wp_kses_data( __("Current tab title", 'fire-department') ),
						"value" => "",
						"type" => "text"
					),
					"_content_" => array(
						"title" => esc_html__("Tab content", 'fire-department'),
						"desc" => wp_kses_data( __("Current tab content", 'fire-department') ),
						"divider" => true,
						"rows" => 4,
						"value" => "",
						"type" => "textarea"
...
-------------------------------------------------------------------- */
if ( !function_exists( 'fire_department_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('fire_department_action_shortcodes_list_vc', 'fire_department_sc_tabs_reg_shortcodes_vc');
	function fire_department_sc_tabs_reg_shortcodes_vc() {
	
		$tab_id_1 = 'sc_tab_'.time() . '_1_' . rand( 0, 100 );
		$tab_id_2 = 'sc_tab_'.time() . '_2_' . rand( 0, 100 );
		vc_map( array(
			"base" => "trx_tabs",
			"name" => esc_html__("Tabs", 'fire-department'),
			"desc" => wp_kses_data( __("Tabs", 'fire-department') ),
			...
			"params" => array(
				array(
					"param_name" => "style",
					"heading" => esc_html__("Tabs style", 'fire-department'),
					"desc" => wp_kses_data( __("Select style of tabs items", 'fire-department') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "initial",
					"heading" => esc_html__("Initially opened tab", 'fire-department'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'fire-department') ),
					"class" => "",
					"value" => 1,
					"type" => "textfield"
				...
				array(
					"param_name" => "scroll",
					"heading" => esc_html__("Scroller", 'fire-department'),
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'fire-department') ),
					"class" => "",
			...
			'default_content' => '
				[trx_tab title="' . esc_html__( 'Tab 1', 'fire-department' ) . '" tab_id="'.esc_attr($tab_id_1).'"][/trx_tab]
				[trx_tab title="' . esc_html__( 'Tab 2', 'fire-department' ) . '" tab_id="'.esc_attr($tab_id_2).'"][/trx_tab]
			',
			"custom_markup" => '
		...
		vc_map( array(
			"base" => "trx_tab",
			"name" => esc_html__("Tab item", 'fire-department'),
			"desc" => wp_kses_data( __("Single tab item", 'fire-department') ),
			...
			"params" => array(
				array(
					"param_name" => "title",
					"heading" => esc_html__("Tab title", 'fire-department'),
					"desc" => wp_kses_data( __("Title for current tab", 'fire-department') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
				...
				array(
					"param_name" => "tab_id",
					"heading" => esc_html__("Tab ID", 'fire-department'),
					"desc" => wp_kses_data( __("ID for current tab (required). Please, start it from letter.", 'fire-department') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
		...
		class WPBakeryShortCode_Trx_Tabs extends FIRE_DEPARTMENT_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends FIRE_DEPARTMENT_VC_ShortCodeTab {}
	}
}
?>