...
// Theme init
if (!function_exists('fire_department_importer_theme_setup')) {
	add_action( 'fire_department_action_after_init_theme', 'fire_department_importer_theme_setup' );		// Fire this action after load theme options
	function fire_department_importer_theme_setup() {
		if (is_admin() && current_user_can('import') && fire_department_get_theme_option('admin_dummy_data')=='yes') {
			new fire_department_dummy_data_importer();
		}
	}
}

class fire_department_dummy_data_importer {
	...
	function __construct() {
		...
		// AJAX handler
		add_action('wp_ajax_fire_department_importer_start_import',		array($this, 'importer'));
		add_action('wp_ajax_nopriv_fire_department_importer_start_import',	array($this, 'importer'));
		// AJAX handler of the get_list_pages actions
		add_action('wp_ajax_fire_department_importer_get_list_pages',		array($this, 'get_list_pages_callback'));
		add_action('wp_ajax_nopriv_fire_department_importer_get_list_pages',array($this, 'get_list_pages_callback'));
		// Check if row will be imported in the set='part'
		add_filter('fire_department_filter_importer_import_row',			array($this, 'import_check_row'), 9, 4);
	}
	...
	function prepare_vars() {
		...
		$this->options['debug'] = fire_department_param_is_on(fire_department_get_theme_option('debug_mode'));
	    $this->options = apply_filters('fire_department_filter_importer_options', $this->options);
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			fire_department_admin_add_menu_item('theme', array(
				'page_title' => esc_html__('Install Dummy Data', 'fire-department'),
				'menu_title' => esc_html__('Install Dummy Data', 'fire-department'),
	...
	function admin_scripts() {
		fire_department_enqueue_style(  'fire_department-importer-style',  fire_department_get_file_url('core/core.importer/core.importer.css'), array(), null );
		fire_department_enqueue_script( 'fire_department-importer-script', fire_department_get_file_url('core/core.importer/core.importer.js'), array('jquery'), null, true );	
	}
	...
	function build_page() {
		...
		if ( isset($_POST['exporter_action']) ) {
			if ( !wp_verify_nonce( fire_department_get_value_gp('nonce'), admin_url() ) )
				$this->error = esc_html__('Incorrect WP-nonce data! Operation canceled!', 'fire-department');
		...
		<div class="trx_importer">
			...
			<div class="trx_importer_section">
				...
				<h2 class="trx_title"><?php esc_html_e('Fire Department Importer', 'fire-department'); ?></h2>

				<form id="trx_importer_form">

					<?php if (count($this->options['files']) > 1) { ?>
						<p><b><?php esc_html_e('Select the demo-data set to be imported:', 'fire-department'); ?></b></p>
					...
					<div class="trx_importer_demo_set">
						<label><input type="radio"<?php if ($this->options['demo_set']=='part') echo ' checked="checked"'; ?> value="part" name="demo_set" /><?php esc_html_e('Only pages, forms and sliders', 'fire-department'); ?></label>
						<div class="trx_importer_description trx_importer_description_part<?php if ($this->options['demo_set']!='part') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>Only pages, forms layouts and sliders will be imported in this case.</b>', 'fire-department')); ?></li>
								<li><?php echo wp_kses_data(__('The imported data will be added to the existing content.', 'fire-department')); ?></li>
								<li><?php echo wp_kses_data(__('All the images are replaced with placeholders.', 'fire-department')); ?></li>
								<li><?php echo wp_kses_data(__('The new pages provide sample markup and shall not be included in the menu! You have to do it yourself.', 'fire-department')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'fire-department')); ?></li>
							</ol>
						</div>
						<label><input type="radio"<?php if ($this->options['demo_set']=='full') echo ' checked="checked"'; ?> value="full" name="demo_set" /><?php esc_html_e('Whole demo-site content', 'fire-department'); ?></label>
						<div class="trx_importer_description trx_importer_description_full<?php if ($this->options['demo_set']!='full') echo ' trx_importer_hidden'; ?>">
							<ol>
								<li><?php echo wp_kses_data(__('<b>It is recommended for new installations of WordPress</b> (without posts, pages and any other data)!', 'fire-department')); ?></li>
								<li><?php echo wp_kses_data(__('As a result, you will get a new set of posts, pages, menu items - <b>a complete copy of our demo site</b>.', 'fire-department')); ?></li>
								<li><?php echo wp_kses_data(__('Import some components (Revolution sliders, Essential Grids, etc.) take a long time - <b>please wait until the end of the procedure, do not navigate away from this page</b>.', 'fire-department')); ?></li>
							</ol>
						</div>
					</div>

					<p><b><?php esc_html_e('Select the elements to be imported:', 'fire-department'); ?></b></p>

					<?php
					$this->show_importer_params(array(
						'slug' => 'posts',
						'title' => esc_html__('Import posts, pages, taxonomies, etc.', 'fire-department'),
						'part' => 1,
						'checked' => true,
					...
					$this->show_importer_params(array(
						'slug' => 'uploads',
						'title' => esc_html__('Import media', 'fire-department'),
						'part' => 0,
						'checked' => true,
						'class' => 'trx_importer_separator'
					));
					$this->show_importer_params(array(
						'slug' => 'tm',
						'title' => esc_html__('Import Theme mods', 'fire-department'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'to',
						'title' => esc_html__('Import Theme Options', 'fire-department'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'tpl',
						'title' => esc_html__('Import Templates Options', 'fire-department'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
						'slug' => 'widgets',
						'title' => esc_html__('Import Widgets', 'fire-department'),
						'part' => 1,
						'checked' => true,
						'class' => 'trx_importer_separator'
					));
					do_action('fire_department_action_importer_params', $this);
					?>

					<div class="trx_buttons">
						<input type="button" value="<?php esc_attr_e('Start import', 'fire-department'); ?>">
					</div>
				</form>
				
				<div id="trx_importer_progress" class="notice notice-info style_<?php echo esc_attr($this->options['demo_style']); ?>">
					<h4 class="trx_importer_progress_title"><?php esc_html_e('Import demo data', 'fire-department'); ?></h4>
					<table border="0" cellpadding="4">
					<?php
					$fields = array(
						'posts'		=> esc_html__('Posts', 'fire-department'),
						'uploads'	=> esc_html__('Media', 'fire-department'),
						'tm'		=> esc_html__('Theme Mods', 'fire-department'),
						'to'		=> esc_html__('Theme Options', 'fire-department'),
						'tpl'		=> esc_html__('Template Options', 'fire-department'),
						'widgets'	=> esc_html__('Widgets', 'fire-department'),
					);
					foreach ($fields as $slug=>$title) {
						$this->show_importer_fields(array('slug' => $slug, 'title' => $title));
					}
					do_action('fire_department_action_importer_import_fields', $this);
					?>
					</table>
					<h4 class="trx_importer_progress_complete"><?php esc_html_e('Congratulations! Data import complete!', 'fire-department'); ?> <a href="<?php echo esc_url(home_url('/')); ?>"><?php esc_html_e('View site', 'fire-department'); ?></a></h4>
				</div>
			...
			<div class="trx_exporter_section">
				...
				<h2 class="trx_title"><?php esc_html_e('Fire Department Exporter', 'fire-department'); ?></h2>

				<?php
				if ($this->error) {
					?><div class="trx_exporter_error notice notice-error"><?php fire_department_show_layout($this->error); ?></div><?php
				}
				?>
				...
				<form id="trx_exporter_form" action="#" method="post">
					...
					if ( isset($_POST['exporter_action']) ) {
						...
						$fields = array(
							'users'			=> esc_html__('Users', 'fire-department'),
							'posts'			=> esc_html__('Posts', 'fire-department'),
							'theme_mods'	=> esc_html__('Theme Mods', 'fire-department'),
							'theme_options'	=> esc_html__('Theme Options', 'fire-department'),
							'templates_options'	=> esc_html__('Templates Options', 'fire-department'),
							'widgets'		=> esc_html__('Widgets', 'fire-department'),
						);
						foreach ($fields as $slug=>$title) {
							$this->show_exporter_fields(array('slug' => $slug, 'title' => $title));
						}
						do_action('fire_department_action_importer_export_fields', $this);
						?></table><?php
						...
						if (false && count($this->options['files']) > 1) {
							...
							?><p><b><?php esc_html_e('Select the demo type to be exported', 'fire-department'); ?></b></p><?php
						...
						?>
						<div class="trx_buttons">
							<input type="submit" value="<?php esc_attr_e('Export Demo Data', 'fire-department'); ?>">
						</div>
						<?php
					}
					?>
	...
	function show_importer_params($args=array()) {
		...
		<label<?php if (!empty($args['class'])) echo ' class="'.esc_attr($args['class']).'"'; ?>>
			<input type="checkbox"
					...
					echo (isset($args['checked']) && $args['checked']) || (in_array($args['slug'], fire_department_storage_get('required_plugins')) && $this->options['plugins_initial_state'])
								? ' checked="checked"'
								: '';
					?>
					value="1"
			...
			<?php fire_department_show_layout($args['title']); ?>
		</label>
		<?php
	}
	...
	function show_importer_fields($args=array()) {
		...
		<tr class="import_<?php echo esc_attr($args['slug']); ?>">
			<td class="import_progress_item"><?php fire_department_show_layout($args['title']); ?></td>
	...
	function show_exporter_fields($args=array()) {
		...
		?>
		<tr>
			<th align="left"><?php fire_department_show_layout($args['title']); ?></th>
			<td><a download="<?php echo esc_attr($args['slug']); ?>.txt" href="<?php echo esc_url(fire_department_get_file_url('core/core.importer/export/'.trim($args['slug']).'.txt')); ?>"><?php esc_html_e('Download', 'fire-department'); ?></a></td>
		</tr>
		<?php
	}
	...
	function check_required_plugins($list='') {
		$not_installed = '';
		if (in_array('trx_utils', fire_department_storage_get('required_plugins')) && !defined('TRX_UTILS_VERSION') )
			$not_installed .= 'Fire Department Utilities';
		$not_installed = apply_filters('fire_department_filter_importer_required_plugins', $not_installed, $list);
		if ($not_installed) {
			$this->error = '<b>'.esc_html__('Attention! For correct installation of the selected demo data, you must install and activate the following plugins: ', 'fire-department').'</b><br>'.($not_installed);
			return false;
		}
		return true;
	}
	...
	function exporter() {
		...
		// Export theme mods
		fire_department_fpc(fire_department_get_file_dir('core/core.importer/export/theme_mods.txt'), serialize($this->prepare_data(get_theme_mods())));

		// Export theme, templates and categories options and VC templates
		$rows = $wpdb->get_results( $wpdb->prepare(
												"SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE %s",
												 fire_department_storage_get('options_prefix') . '_options%'
												 )
									);
		$options = array();
		if (is_array($rows) && count($rows) > 0) {
			foreach ($rows as $row) {
				$options[$row->option_name] = fire_department_unserialize($row->option_value);
			}
		}
		...
		fire_department_fpc(fire_department_get_file_dir('core/core.importer/export/theme_options.txt'), serialize($this->prepare_data($options)));

		// Export templates options
		$rows = $wpdb->get_results( $wpdb->prepare(
												"SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE %s",
												fire_department_storage_get('options_prefix').'_options_template_%'
												)
									);
		$options = array();
		if (is_array($rows) && count($rows) > 0) {
		...
		fire_department_fpc(fire_department_get_file_dir('core/core.importer/export/templates_options.txt'), serialize($this->prepare_data($options)));
		...
		fire_department_fpc(fire_department_get_file_dir('core/core.importer/export/widgets.txt'), serialize($this->prepare_data($options)));

		// Export posts
		fire_department_fpc(fire_department_get_file_dir('core/core.importer/export/posts.txt'), serialize(array(
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				$rows[$k]['user_pass']		= '';
				$rows[$k]['display_name']	= sprintf(esc_html__('User %d', 'fire-department'), $v['ID']);
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $k=>$v) {
				...
				else if ($v['meta_key'] == 'first_name')			$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'fire-department'), $v['user_id']);
				else if ($v['meta_key'] == 'last_name')				$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'fire-department'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_first_name')	$rows[$k]['meta_value'] = sprintf(esc_html__('FName%d', 'fire-department'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_last_name')		$rows[$k]['meta_value'] = sprintf(esc_html__('LName%d', 'fire-department'), $v['user_id']);
				else if ($v['meta_key'] == 'billing_email')			$rows[$k]['meta_value'] = 'user'.$v['user_id'].'@user-mail.net';
			}
		}
		$users['usermeta'] = $rows;
		fire_department_fpc(fire_department_get_file_dir('core/core.importer/export/users.txt'), serialize($users));

		// Export Theme specific post types
		do_action('fire_department_action_importer_export', $this);
	...
	function export_dump($table) {
		...
		if ( count( $wpdb->get_results( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->prefix . trim($table) ), ARRAY_A ) ) == 1 ) {
			...
			if ($this->options['debug']) dfl(sprintf(__("Export %d rows from table '%s'. Used query: %s", 'fire-department'), count($rows), $table, $query));
		}
		return $rows;
	}
	...
	// Import dummy data
	//-----------------------------------------------------------------------------------
	//Handler of the add_action('wp_ajax_fire_department_importer_start_import',			array($this, 'importer'));
	//Handler of the add_action('wp_ajax_nopriv_fire_department_importer_start_import',	array($this, 'importer'));
	function importer() {

		if ($this->options['debug']) dfl(__('AJAX handler for importer', 'fire-department'));

		if ( !isset($_POST['importer_action']) || !wp_verify_nonce( fire_department_get_value_gp('ajax_nonce'), admin_url('admin-ajax.php') ) )
			die();
		...
		if ($this->options['debug']) dfl( sprintf(__('Dispatch action: %s', 'fire-department'), $this->action) );
		...
		$admin_tm = max(0, min(1800, (int) fire_department_get_theme_option('admin_dummy_timeout')));
		...
		// Import posts and users
			...
			if ($this->response['result'] >= 100 && $this->options['demo_set']=='full') {
				$this->import_users();
				do_action('fire_department_action_importer_after_import_posts', $this);
			}
		...
		// Import Theme specific posts
			...
			do_action('fire_department_action_importer_import', $this, $this->action);
		}
		...
		if ($this->options['debug']) dfl( sprintf(__("AJAX handler finished - send results to client: %s", 'fire-department'), json_encode($this->response)) );
	...
	function clear_tables() {
		...
		if ($this->options['demo_set']=='full') {
			if (fire_department_strpos($_POST['demo_parts'], 'posts')!==false) {
				if ($this->options['debug']) dfl( __('Clear tables', 'fire-department') );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->posts}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "posts".', 'fire-department' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->postmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "postmeta".', 'fire-department' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->comments}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "comments".', 'fire-department' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->commentmeta}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "commentmeta".', 'fire-department' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->terms}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "terms".', 'fire-department' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_relationships}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "term_relationships".', 'fire-department' ) . ' ' . ($res->get_error_message()) );
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->term_taxonomy}");
				if ( is_wp_error( $res ) ) dfl( __( 'Failed truncate table "term_taxonomy".', 'fire-department' ) . ' ' . ($res->get_error_message()) );
			}
			do_action('fire_department_action_importer_clear_tables', $this, $_POST['clear_tables']);
		}
	}
	...
	function import_users() {
		if ($this->options['debug'])
			dfl(__('Import users', 'fire-department'));
		$this->response['start_from_id'] = 0;
		$this->import_dump('users', esc_html__('Users', 'fire-department'));
	}

	// Import posts, terms and comments
	function import_posts() {
		if ($this->options['debug'])
			dfl(__('Import posts, terms and comments', 'fire-department'));
		...
		$this->import_dump('posts', esc_html__('Posts', 'fire-department'));
	}

	// Import media (uploads folder)
	function import_uploads() {
		if ($this->options['debug'])
			dfl(__('Import media', 'fire-department'));
		...
		if ( ($txt = get_option('fire_department_import_uploads')) == '' ) {
			if ( ($txt = $this->get_file($this->options['files'][$this->options['demo_type']]['file_with_uploads'])) === false)
				return;
			else
				update_option('fire_department_import_uploads', $txt);
		}
		$files = fire_department_unserialize($txt);
		...
		foreach ($files as $file) {
			...
			if (fire_department_get_file_ext($file)=='zip') {
			...
			} else if (substr(fire_department_get_file_ext($file), 0, 1)==='0' && (int) fire_department_get_file_ext($file) > 0) {
				if ( ($txt = $this->get_file($file, round(max(0, $counter-1) / count($files) * 100))) === false)
					$need_exit = true;
				else {
					$zip = $this->uploads_dir.'/import_media.tmp';
					fire_department_fpc($zip, $txt, $file==$files[0] ? 0 : FILE_APPEND);
			...
			// Check time
			if ($this->options['debug'])
				dfl(sprintf( __('File %s imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'fire-department'), $file, $result.'%', $this->max_time, time() - $this->start_time));
			// Unzip file
			if ($need_extract) {
				if (!empty($zip) && file_exists($zip)) {
					if ($this->options['debug'])
						dfl(sprintf(__('Extract zip-file "%s"', 'fire-department'), $zip));
					if ( !unzip_file($zip, $this->uploads_dir) ) {
						$msg = sprintf(__('Error when unzip file "%s"', 'fire-department'), $zip);
					...
					$msg = sprintf(__('File "%s" not found', 'fire-department'), $zip);
					$this->response['error'] = $msg;
					if ($this->options['debug'])
						dfl($msg);
				}
		...
		if ($result >= 100) update_option('fire_department_import_uploads', '');
	...
	function import_theme_mods() {
		if ($this->options['debug'])
			dfl(__('Import Theme mods', 'fire-department'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'fire-department'), $this->options['files'][$this->options['demo_type']]['file_with_mods']));
		}
	}
	...
	function import_theme_options() {
		if ($this->options['debug'])
			dfl(__('Import Theme Options', 'fire-department'));
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $k=>$v) {
				...
				update_option( $k, apply_filters('fire_department_filter_import_theme_options', $v, $k) );
			}
		} else {
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'fire-department'), $this->options['files'][$this->options['demo_type']]['file_with_options']));
		}
	}
	...
	function import_widgets() {
		if ($this->options['debug'])
			dfl(__('Import Widgets', 'fire-department'));
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'fire-department'), $this->options['files'][$this->options['demo_type']]['file_with_widgets']));
		}
	}
	...
	function import_dump($slug, $title) {
		if ($this->options['debug'])
			dfl(sprintf(__('Import dump file: "%s"', 'fire-department'), $this->options['files'][$this->options['demo_type']]['file_with_' . $slug]));
		...
		if (is_array($data) && count($data) > 0) {
			...
			foreach ($data as $table=>$rows) {
				if ($this->options['debug']) dfl(sprintf(__('Process table "%s"', 'fire-department'), $table));
				...
				if ($this->options['demo_set']=='part' && $table=='posts' && $this->response['start_from_id'] > 0) {
					$this->part_replace = get_option('fire_department_importer_part_replace', array());
					$this->part_image = get_option('fire_department_importer_part_image', array());
				}
				if (is_array($rows) && ($posts_all=count($rows)) > 0) {
					...
					foreach ($rows as $row) {
						...
						if (!apply_filters('fire_department_filter_importer_import_row', $this->options['demo_set']=='full', $table, $row, $this->options['demo_parts'])) continue;
						// Replace demo URL to current site URL
						$row = fire_department_replace_site_url($row, $this->options['files'][$this->options['demo_type']]['domain_demo']);
						$f = '';
						$v = '';
						...
						if ($this->options['debug']) {
							dfl( sprintf( __('Record (ID=%s) is imported. Progress: %s. Time: %s sec. from %s sec.', 'fire-department'),
											!empty($row['ID'])
				...
				if ($this->options['demo_set']=='part' && $table=='posts') {
					update_option('fire_department_importer_part_replace', $result < 100 ? $this->part_replace : array());
					update_option('fire_department_importer_part_image', $result < 100 ? $this->part_image : array());
				}
				if ($this->options['debug']) dfl(sprintf(__('Imported %s. Elapsed time %s sec. of %s sec.', 'fire-department'), $result.'%', time() - $this->start_time, $this->max_time));
				if ($break) break;
			}
		} else {
			if ($this->options['debug'])
				dfl(sprintf(__('Error unserialize data from the file %s', 'fire-department'), $this->options['files'][$demo_type]['file_with_' . $slug]));
		}
		$this->response['result'] = $result;
	}

	// Check if the row will be imported
	// Handler of the add_filter('fire_department_filter_importer_import_row', array($this, 'import_check_row'), 9, 4);
	...
	function import_prepare_no_image() {
		$no_image_title = esc_html__('No-Image placeholder', 'fire-department');
		$no_image_post = get_page_by_title($no_image_title, OBJECT, 'attachment');
		if ( empty($no_image_post->ID) ) {
			if ( ($img = fire_department_get_file_dir("images/no-image.jpg")) != '') {
				// Copy to the 'uploads' folder
				$this->part_image = wp_upload_bits( 'no-image.jpg', 0, fire_department_fgc($img));
	...
	function get_list_pages_callback() {
		if ( !wp_verify_nonce( fire_department_get_value_gp('ajax_nonce'), admin_url('admin-ajax.php') ) )
			die();

		$this->prepare_vars();

		$response = array(
			'error' => empty($_POST['demo_type']) ? esc_html__('Incorrect parameters', 'fire-department') : '',
		);
	...
	function replace_uploads($str) {
		return fire_department_replace_uploads_url($str);
	}
	...
	function prepare_data($str) {
		$need_ser = false;
		if (is_string($str) && fire_department_substr($str, 0, 2)=='a:') {
		...
		if (is_array($str) && count($str) > 0) {
			...
			if ($this->options['files'][$this->options['demo_type']]['domain_dev']!=$this->options['files'][$this->options['demo_type']]['domain_demo'])
				$str = str_replace(
							fire_department_get_domain_from_url($this->options['files'][$this->options['demo_type']]['domain_dev']),
							fire_department_get_domain_from_url($this->options['files'][$this->options['demo_type']]['domain_demo']),
							$str);
	...
	function download_file($fname, $result=0) {
		...
		if (substr($fname, 0, 5)=='http:' || substr($fname, 0, 6)=='https:') {
			...
			if (is_string($response)) {
				...
				if ($this->options['debug'])
					dfl(sprintf(__('Download file %s successful', 'fire-department'), $fname));
			} else {
				if ($attempt < 3) {
					$this->response['attempt'] = $attempt;
					$this->response['result'] = $result;
					if ($this->options['debug']) {
						$error_log = sprintf(__("Attempt %d. Download file '%s' failed.", 'fire-department'), $attempt, $fname);
						dfl($error_log);
					}
				} else {
					unset($this->response['attempt']);
					$this->response['error'] = sprintf(__("Error download file '%s'.", 'fire-department'), $fname)
												. " \n" . __("Please, try again!", 'fire-department');
					if ($this->options['debug'])
			...
			$rez = fire_department_get_file_dir($fname);
		}
		return $rez;
	}
	...
	function get_file($fname, $result=0) {
		$attempt = !empty($_POST['attempt']) ? (int) $_POST['attempt']+1 : 1;
		$fname = trailingslashit($this->options['demo_url']) . trim($this->options['demo_type']) . '/' . trim($fname);
		$txt = fire_department_fgc($fname, true);
		if (empty($txt)) {
			if ($attempt < 3) {
				$this->response['attempt'] = $attempt;
				$this->response['result'] = $result;
				if ($this->options['debug']) {
					$error_log = sprintf(__("Attempt %d. Load data from the file '%s' failed. ", 'fire-department'), $attempt, $fname);
					dfl($error_log);
				}
			} else {
				unset($this->response['attempt']);
				$this->response['error'] = sprintf(__("Error load data from the file '%s'.", 'fire-department'), $fname)
											. " \n" . __("Please, try again!", 'fire-department');
				if ($this->options['debug'])
					dfl($this->response['error']);
			}
			...
			unset($this->response['attempt']);
			if ($this->options['debug'])
				dfl(sprintf(__('Load data from the file %s successful', 'fire-department'), $fname));
		}
		return $txt;
	}
}
