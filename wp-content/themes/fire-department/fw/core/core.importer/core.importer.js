// Fire Department Importer script
...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_demo_type input[type="radio"]').on('change', function() {
		...
		var data = {
			ajax_nonce: FIRE_DEPARTMENT_STORAGE['ajax_nonce'],
			action: 'fire_department_importer_get_list_pages',
			demo_type: type
		};
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		FIRE_DEPARTMENT_STORAGE['importer_error_messages'] = '';
		FIRE_DEPARTMENT_STORAGE['importer_ignore_errors'] = true;
		fire_department_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function fire_department_importer_do_action(steps, idx) {
	"use strict";
	...
	var data = {
		ajax_nonce: FIRE_DEPARTMENT_STORAGE['ajax_nonce'],
		action: 'fire_department_importer_start_import',
	...
	jQuery.post(FIRE_DEPARTMENT_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || FIRE_DEPARTMENT_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '')
				FIRE_DEPARTMENT_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				fire_department_importer_do_action(steps, idx);
			} else {
				if (FIRE_DEPARTMENT_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + FIRE_DEPARTMENT_STORAGE['importer_error_msg'] + '</h4>' + FIRE_DEPARTMENT_STORAGE['importer_error_messages']);
				} else {
