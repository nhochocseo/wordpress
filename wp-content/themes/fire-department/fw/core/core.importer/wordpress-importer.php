...
Version: 0.6.1 (modified by Fire Department - no backward compatibility!)
...
class WP_Import extends WP_Importer {
	...
	function import( $file ) {
		...
		@set_time_limit(max(30, min(1800, (int) fire_department_get_theme_option('admin_dummy_timeout'))));
		...
		if ($this->debug) dfl(esc_html__('Process authors', 'fire-department'));
		...
		try {
			...
			$error = true;
			dfl( sprintf(esc_html__('Error while import: %s', 'fire-department'), $e->getMessage()) );
	...
	function import_start( $file ) {
		...
		if ($this->debug) dfl(esc_html__('Start import posts', 'fire-department'));

		if ( ! is_file($file) ) {
			dfl( sprintf(esc_html__('Sorry, there has been an error: %s', 'fire-department'), esc_html__( 'The file does not exist, please try again.', 'fire-department')) );
			$this->footer();
			die();
		}
		...
		if ( is_wp_error( $import_data ) ) {
			dfl( sprintf(esc_html__('Sorry, there has been an error: %s', 'fire-department'), $import_data->get_error_message()) );
			$this->footer();
			die();
		}
	...
	function import_end() {
		...
		if ($this->debug) dfl(esc_html__('Finish import posts', 'fire-department'));
		...
		if ($this->debug) dfl(esc_html__('All done', 'fire-department'));
	...
	function get_authors_from_import( $import_data ) {
		...
		if ($this->debug) dfl( esc_html__('Get authors from import', 'fire-department') );
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					if ($this->debug) dfl( sprintf(esc_html__( 'Failed to import author "%s". Their posts will be attributed to the current user.', 'fire-department' ), $post['post_author']) );
					continue;
				}
	...
	function get_author_mapping() {
		...
		if ($this->debug) dfl( esc_html__('Author mapping', 'fire-department') );
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ($this->debug) dfl( sprintf(esc_html__('Author "%s" imported.', 'fire-department'), $this->authors[$old_login]['author_display_name']) );
				} else {
					if ($this->debug) dfl( sprintf(esc_html__('Failed to create new user for %s. Their posts will be attributed to the current user. Message: %s', 'fire-department'), $this->authors[$old_login]['author_display_name'], $user_id->get_error_message()) );
				}
			}
	...
	function process_categories() {
		...
		if ($this->debug) dfl( esc_html__('Process categories', 'fire-department') );
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($cat['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf( esc_html__( 'Category "%s" imported.', 'fire-department' ).'<br>', esc_html($cat['category_nicename']) ) );
				}
			} else {
				dfl( sprintf( esc_html__( 'Failed to import category "%s"', 'fire-department' ), esc_html($cat['category_nicename']), $id->get_error_message() ) );
				continue;
			}
		}
	...
	function process_tags() {
		...
		if ($this->debug) dfl( esc_html__('Process tags', 'fire-department') );
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($tag['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf( esc_html__( 'Tag "%s" imported.', 'fire-department' ), $tag['tag_name']) );
				}
			} else {
				dfl( sprintf( esc_html__( 'Failed to import post tag "%s". Message: %s', 'fire-department' ), $tag['tag_name'], $id->get_error_message()) );
				continue;
			}
		}
	...
	function process_terms() {
		...
		if ($this->debug) dfl( esc_html__('Process terms', 'fire-department') );
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($term['term_id']) ) {
					...
					if ( $this->debug ) dfl( sprintf(esc_html__( 'Taxonomy "%s": term "%s" imported.', 'fire-department' ), $term['term_taxonomy'], $term['term_name']) );
				}
			} else {
				dfl( sprintf(esc_html__( 'Failed to import term %s: "%s". Message: %s', 'fire-department' ), $term['term_taxonomy'], $term['term_name'], $id->get_error_message()) );
				continue;
			}
		}
	...
	function process_posts() {
		...
		if ($this->debug) dfl( sprintf(esc_html__('Process posts %s', 'fire-department'), $this->start_from_id > 0 
																						? sprintf(esc_html__('(continue from ID = %s)', 'fire-department'), $this->start_from_id) 
																						: ''
											)
								);
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				...
				dfl( sprintf( esc_html__( 'Failed to import post "%s": Invalid post type "%s"', 'fire-department' ), $post['post_title'], $post['post_type']) );
				...
				do {	
					...
					if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
						dfl( sprintf(esc_html__('%s "%s" already exists.', 'fire-department'), $post_type_object->labels->singular_name, $post['post_title']) );
						...
						if ( is_wp_error( $post_id ) ) {
							dfl( sprintf(esc_html__( 'Failed to import %s: "%s". Message: %s', 'fire-department' ), $post_type_object->labels->singular_name, $post['post_title'], $post_id->get_error_message()) );
							break;
						}
						...
						if ( $this->debug ) dfl( sprintf( esc_html__( '%s "%s" (ID=%s) imported.', 'fire-department' ), $post_type_object->labels->singular_name, $post['post_title'], $post_id) );
					}
						...
						foreach ( $post['terms'] as $term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									dfl( sprintf( esc_html__( 'Failed to import post term %s: "%s". Message: %s', 'fire-department' ), $taxonomy, $term['name'], $t->get_error_message()) );
						...
						foreach ( $newcomments as $key => $comment ) {
							...
							if ( ! $post_exists || ! comment_exists( $comment['comment_author'], $comment['comment_date'] ) ) {
								...
								foreach( $comment['commentmeta'] as $meta ) {
									$value = fire_department_unserialize( $meta['value'] );
							...
							if ( $key ) {
								...
								if ( ! $value ) {
									...
									if (is_serialized($value)) {
										$value = fire_department_unserialize( $value );
										if ( $value===false ) {
											if ($this->debug) dfl( sprintf(esc_html__('Post (ID=%s) - error unserialize postmeta: %s', 'fire-department'), $post['post_id'], $key) );
											$value = $meta['value'];
											$replace = false;
										}
									}
			...
			fire_department_fpc($this->import_log, trim(max($original_post_ID, $this->start_from_id)) . '|' . trim($result));
			if ($this->debug) dfl( sprintf( esc_html__('Post (ID=%s) imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'fire-department'), $post['post_id'], $result.'%', $this->max_time, time() - $this->start_time) );
	...
	function process_menu_item( $item ) {
		...
		if ($this->debug) dfl( esc_html__('Process menu item', 'fire-department') );
		...
		if ( ! $menu_slug ) {
			if ($this->debug) dfl( sprintf(esc_html__( 'Menu item "%s" (id=%s) skipped due to missing menu slug', 'fire-department' ), $item['post_title'], $item['post_id']) );
			return;
		}

		$menu_id = term_exists( $menu_slug, 'nav_menu' );
		if ( ! $menu_id ) {
			if ($this->debug) dfl( sprintf( esc_html__( 'Menu item "%s" (id=%s) skipped due to invalid menu slug: %s', 'fire-department' ), $item['post_title'], $item['post_id'], $menu_slug) );
			return;
		} else {
		...
		$_menu_item_classes = fire_department_unserialize( $_menu_item_classes );
		...
		if ($_menu_item_type == 'custom' && strpos($_menu_item_url, $this->demo_url) !== false) {
			$_menu_item_url = str_replace($this->demo_url, get_site_url() . (fire_department_substr($this->demo_url, -1)=='/' ? '/' : ''), $_menu_item_url);
		}
		...
		if ( $id && ! is_wp_error( $id ) ) {
			if ($this->overwrite) {
				...
				if ( ! empty( $item['postmeta'] ) ) {
					foreach ( $item['postmeta'] as $meta ) {
						...
						if (fire_department_substr($key, 0, 11)=='_menu_item_') continue;
						...
						if ( $key ) {
							...
							if ( ! $value ) {
								...
								if (is_serialized($value)) {
									...
									if ( $value === false ) {
										if ($this->debug) {
											dfl(sprintf( esc_html__('Menu item (ID=%s) - error unserialize postmeta: %s=', 'fire-department'), $item['post_id'], $key) );
										}
										$value = $meta['value'];
										$replace = false;
									}
			...
			if ( $this->debug ) dfl( sprintf( esc_html__( 'Menu item "%s" (ID=%s) imported.', 'fire-department'), $item['post_title'], $item['post_id']) );
		}
	}
	...
	function process_attachment( $post, $url ) {
		...
		if ($this->debug) dfl( esc_html__('Process attachment', 'fire-department') );
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			$msg = esc_html__('Invalid file type', 'fire-department');
			dfl( $msg );
	...
	function fetch_remote_file( $url, $post ) {
        ...
        if ( $code !== 200 ) {
            ...
            return new WP_Error(
                'import_file_error',
                sprintf(
                    __('Remote server returned %1$d %2$s for %3$s', 'fire-department'),
                    $code,
        ...
        if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
            @unlink( $upload['file'] );
            return new WP_Error( 'import_file_error', __('Remote file is incorrect size', 'fire-department') );
        }

        if ( 0 == $filesize ) {
            @unlink( $upload['file'] );
            return new WP_Error( 'import_file_error', __('Zero size file downloaded', 'fire-department') );
        }

        $max_size = (int) $this->max_attachment_size();
        if ( ! empty( $max_size ) && $filesize > $max_size ) {
            @unlink( $upload['file'] );
            return new WP_Error( 'import_file_error', sprintf(__('Remote file is too large, limit is %s', 'fire-department'), size_format($max_size) ) );
        }
	...
	function replace_uploads($str) {
		return fire_department_replace_uploads_url($str, $this->uploads_folder);
	}
	...
	function backfill_parents() {
		...
		if ($this->debug) dfl(esc_html__('Backfill parents', 'fire-department'));
	...
	function backfill_attachment_urls() {
		...
		if ($this->debug) dfl(esc_html__('Backfill attachments', 'fire-department'));
	...
	function remap_featured_images() {
		...
		if ($this->debug) dfl(esc_html__('Remap featured images', 'fire-department'));
	 ...
	 */
	function parse( $file ) {
		if ($this->debug) dfl( sprintf(esc_html__('Parse file %s', 'fire-department'), $file) );
