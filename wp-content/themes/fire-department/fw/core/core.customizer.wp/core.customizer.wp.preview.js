...
( function( $ ) {
	...
	if (!fire_department_previewer_vars['need_refresh']) {
		var $style = $('#fire_department-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="fire_department-customizer-css"></'+'style'+'>' ).find('#fire_department-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			fire_department_set_cookie(fire_department_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
