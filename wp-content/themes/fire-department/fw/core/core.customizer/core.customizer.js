// Fire Department Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.fire_department_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.fire_department_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && fire_department_get_cookie('to_demo_fixed')==1) fire_department_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.fire_department_options_field_info .to_demo_pin', function(e) {
		fire_department_options_fix_demo(jQuery(this).parents('.fire_department_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	fire_department_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.fire_department_options_input,.fire_department_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.fire_department_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		fire_department_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.fire_department_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.fire_department_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .fire_department_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.fire_department_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? FIRE_DEPARTMENT_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.fire_department_options_tab').find('> .fire_department_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="fire_department_options_schemes_order" class="fire_department_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function fire_department_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.fire_department_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.fire_department_options').find('>.fire_department_options_field_info');
...
function fire_department_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.fire_department_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	fire_department_message_confirm(FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_delete_confirm'], FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'fire_department_options_scheme_delete',
			nonce: FIRE_DEPARTMENT_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.fire_department_options_tab').find('> .fire_department_options_schemes_order').val()
		};
		setTimeout(function(){
			fire_department_message_info(FIRE_DEPARTMENT_STORAGE['to_strings']['recompile_styles'], FIRE_DEPARTMENT_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(FIRE_DEPARTMENT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				fire_department_message_success(FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+FIRE_DEPARTMENT_STORAGE['to_strings']['reload_page'], FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				fire_department_message_warning(FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function fire_department_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.fire_department_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	fire_department_message_confirm(FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_copy_confirm'], FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'fire_department_options_scheme_copy',
			nonce: FIRE_DEPARTMENT_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.fire_department_options_tab').find('> .fire_department_options_schemes_order').val()
		};
		...
		jQuery.post(FIRE_DEPARTMENT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				fire_department_message_success(FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+FIRE_DEPARTMENT_STORAGE['to_strings']['reload_page'], FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				fire_department_message_warning(FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, FIRE_DEPARTMENT_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function fire_department_options_fix_demo(info) {
	var info2 = info.parents('.fire_department_options').toggleClass('to_demo_fixed').find('>.fire_department_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.fire_department_options').append(clonedObj);
		info2 = info.parents('.fire_department_options').find('>.fire_department_options_field_info');
	}
	fire_department_set_cookie('to_demo_fixed', jQuery('.fire_department_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function fire_department_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.fire_department_options_field_info').data('scheme');
