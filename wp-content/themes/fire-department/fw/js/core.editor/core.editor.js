jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(FIRE_DEPARTMENT_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == FIRE_DEPARTMENT_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(FIRE_DEPARTMENT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				fire_department_message_success('', FIRE_DEPARTMENT_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(FIRE_DEPARTMENT_STORAGE['strings']['editor_caption_close']);
			} else {
				fire_department_message_warning(rez.error, FIRE_DEPARTMENT_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		fire_department_message_confirm(FIRE_DEPARTMENT_STORAGE['strings']['editor_delete_post'], FIRE_DEPARTMENT_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(FIRE_DEPARTMENT_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					fire_department_message_success('', FIRE_DEPARTMENT_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = FIRE_DEPARTMENT_STORAGE['site_url'];
						}, 1000);
				} else {
					fire_department_message_warning(rez.error, FIRE_DEPARTMENT_STORAGE['strings']['editor_delete_error']);
				}
			});
