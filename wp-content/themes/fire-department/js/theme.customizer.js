...
function fire_department_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = FIRE_DEPARTMENT_STORAGE['co_add_params']!=undefined ? FIRE_DEPARTMENT_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#fire_department-customizer-styles-'+option).length > 0 ? jQuery('#fire_department-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="fire_department-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
